...
const scheduleAssignSchema = z.object({
  engagementId: z.string().uuid(),
  resourceId: z.string().uuid()
});

export async function PATCH(request: Request) {
  const auth = await requireApiPermission("schedule:manage");
  if ("error" in auth) return auth.error;

  const parsed = scheduleAssignSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
  ...
  if (!engagement.scheduledStart || !engagement.scheduledEnd) {
    return NextResponse.json({ error: "Schedule the engagement before assigning a resource." }, { status: 400 });
  }
